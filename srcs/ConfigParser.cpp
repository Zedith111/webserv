...
int	ConfigParser::parseServer(size_t &current, int indent_level, serverConf *current_conf){
	...
	func	key_funcs[key_size] = {&ConfigParser::parseListen, &ConfigParser::parseServerName, &ConfigParser::parseRoot, &ConfigParser::parseCGIBin,
...
int ConfigParser::parseCGIBin(size_t &current, serverConf *current_conf){
	...
	std::string path = this->tokens[current];
	if (checkIsDirectory(path) != 1){
		std::cout << COLOR_RED << "Error. Invalid cgi_bin path: " << path << COLOR_RESET << std::endl;
		return (0);
	}
...
int ConfigParser::parseCGIHandler(size_t &current, serverConf *current_conf){
	current += 1;
...
int ConfigParser::initDefaultErrorpages(){
	this->default_error_pages[400] = "./default_error_pages/400.html";
